...
//    int num_blotches = rand_gen.randint(0, 6);
//    for (int j = 0; j < num_blotches; j++) {
//          float rx = rand_gen.rand01() * rect.width();
//          float ry = rand_gen.rand01() * rect.height();
//          float rdx = (rand_gen.rand01() * max_rand_dim + min_rand_dim) * rect.width();
//          float rdy = (rand_gen.rand01() * max_rand_dim + min_rand_dim) * rect.height();
//
//          QRectF dst3 = QRectF(rx, ry, rdx, rdy);
//          p.fillRect(dst3, QColor(rand_gen.randint(0, 255), rand_gen.randint(0, 255), rand_gen.randint(0, 255)));
//
//        }

    if (options.use_data_augmentation) {
        float max_rand_dim = .25;
        float min_rand_dim = .1;
//        int num_blotches = rand_gen.randint(0, 6);
//        int num_blotches = 6;
        float num_blotches = rand_gen.rand01() *5.0;
        for (int j = 0; j < num_blotches; j++) {
              float rx = rand_gen.rand01() * rect.width();
              float ry = rand_gen.rand01() * rect.height();
              float rdx = (rand_gen.rand01() * max_rand_dim + min_rand_dim) * rect.width();
