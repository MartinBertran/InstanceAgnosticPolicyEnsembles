...
lib.vec_wait.argtypes = [
    ...
    npct.ndpointer(dtype=np.int32, ndim=1),     # lvl idx TODO MB!
    ]
...
def init_args_and_threads(cpu_count=4,
    ...
    int_args = np.array([
        ...
        game_versions[Config.GAME_TYPE], Config.SET_SEED, rand_seed,
        Config.SINGLE_THEME, #TODO MB!
    ]).astype(np.int32)
...
class CoinRunVecEnv(VecEnv):
    ...
    def __init__(self, game_type, num_envs, lump_n=0, default_zoom=5.0):
        ...
        self.buf_lvl = np.zeros([num_envs], dtype=np.int32)
    ...
    def step_wait(self):
        ...
        self.buf_lvl = np.zeros_like(self.buf_lvl)
        ...
        self.dummy_info = [{'level':self.buf_lvl[_]} for _ in range(self.buf_lvl.shape[0])]
