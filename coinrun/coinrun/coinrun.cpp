...
bool USE_UNIFIED_THEME = false; //TODO MB
...
struct State {
  ...
  int ground_n;
  int bg_n;
  int lvl_seed; //TODO REVIEW MARTIN
...
void state_reset(const std::shared_ptr<State>& state, int game_type)
{
  ...
  state->lvl_seed = level_seed; //TODO REVIEW MARTIN
...
void initialize_args(int *int_args) {
  ...
  USE_UNIFIED_THEME = int_args[7] == 1; //TODO MB
  ...
  int* lvls) //TODO REVIEW MARTIN
{
  ...
  for (int e = 0; e < vstate->nenvs; e++) {
    ...
    lvls[e] = lvl_seed; //TODO REVIEW MARTIN
    a.reward = 0;
    a.game_over = false;
  }
}
...
public:
  ...
  void timeout()
  {
    ...
    float bufrew[1];
    bool bufdone[1];
    int buflvl[1]; //TODO REVIEW MARTIN
    vec_wait(viz->control_handle, bufrgb, 0, bufrew, bufdone, buflvl);//TODO REVIEW MARTIN
